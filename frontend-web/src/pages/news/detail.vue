    ...
    <el-container>
      ...
      <el-main v-loading="loading" class="detail-main">
        <template v-if="detail">
          ...
          <!-- AI 摘要 -->
          <div v-if="detail.aiAnalysis" class="ai-section">
            <h3 class="section-title">🤖 {{ $t('news.summary') }}</h3>
            <p>{{ detail.aiAnalysis.summary || detail.summary }}</p>
          </div>

          <!-- 核心事实 -->
          <div v-if="detail.aiAnalysis?.summary" class="content-section">
...
import type { NewsDetail, NewsItem as NewsItemType } from '@/stores/newsStore';
...
<style scoped>
.detail-page { min-height: 100vh; background: var(--bg-secondary); }
.detail-header {
...
.fav-btn { font-size: 22px; cursor: pointer; }
.detail-main { max-width: 760px; margin: 0 auto; padding: 32px 16px; }
.detail-title { font-size: 28px; font-weight: 700; line-height: 1.4; margin-bottom: 16px; }
.detail-meta { display: flex; gap: 16px; font-size: 14px; color: var(--text-secondary); margin-bottom: 32px; flex-wrap: wrap; }
.ai-section { background: linear-gradient(135deg, #e8f4fd, #f0e6ff); padding: 20px; border-radius: 12px; margin-bottom: 24px; }
